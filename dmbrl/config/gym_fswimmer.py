...
class FixedSwimmerConfigModule:
    """
    """
    ...
    PLAN_HOR = 30
    INIT_VAR = 0.25
    MODEL_IN, MODEL_OUT = 11, 9  # obs - > 8 + 1, action 2
    GP_NINDUCING_POINTS = 300

    def __init__(self):
        # self.ENV = gym.make(self.ENV_NAME)
        from mbbl.env.gym_env import fixed_swimmer
        self.ENV = fixed_swimmer.env(env_name='gym_fswimmer', rand_seed=1234,
    ...
    def obs_cost_fn(obs):
        ...
        """
        if isinstance(obs, np.ndarray):
            velocity_cost = -obs[:, -1]  # the qvel for the root-x joint
...
CONFIG_MODULE = FixedSwimmerConfigModule
