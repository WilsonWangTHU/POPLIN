...
class FixedHopperConfigModule:
    ENV_NAME = "MBRLGYM_Hopper-v0"
    TASK_HORIZON = 1000
    NTRAIN_ITERS = 300
    NROLLOUTS_PER_ITER = 1
    PLAN_HOR = 30
    INIT_VAR = 0.25
    MODEL_IN, MODEL_OUT = 14, 11  # obs - > 11, action 3
    GP_NINDUCING_POINTS = 300

    def __init__(self):
        # self.ENV = gym.make(self.ENV_NAME)
        from mbbl.env.gym_env import fixed_walker
        self.ENV = fixed_walker.env(env_name='gym_fhopper', rand_seed=1234,
    ...
    def obs_cost_fn(obs):
        ...
        """
        if isinstance(obs, np.ndarray):
            velocity_cost = -obs[:, 5]  # the qvel for the root-x joint
            height_cost = 3 * np.square(obs[:, 0] - 1.3)  # the height
            # height, ang = ob[0], ob[1]
            done = (obs[:, 0] <= 0.7) or (abs(obs[:, 1]) >= 0.2)
            alive_reward = 1.0 - np.array(done, dtype=np.float)
            return velocity_cost + height_cost - alive_reward
        else:
            velocity_cost = -obs[:, 5]  # the qvel for the root-x joint
            height_cost = 3 * tf.square(obs[:, 0] - 1.3)  # the height
            done = tf.logical_or(obs[:, 0] <= 0.7, tf.abs(obs[:, 1]) >= 0.2)
            alive_reward = 1.0 - tf.cast(done, dtype=velocity_cost.dtype)
            return velocity_cost + height_cost - alive_reward
...
CONFIG_MODULE = FixedHopperConfigModule
