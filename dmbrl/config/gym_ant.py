...
class AntConfigModule:
    ...
    """
    ENV_NAME = "MBRLGYM_ANT-v0"
    TASK_HORIZON = 1000
    NTRAIN_ITERS = 300
    NROLLOUTS_PER_ITER = 1
    PLAN_HOR = 30
    INIT_VAR = 0.25
    MODEL_IN, MODEL_OUT = 35, 27  # obs - > 27, action 8
    GP_NINDUCING_POINTS = 300

    def __init__(self):
        # self.ENV = gym.make(self.ENV_NAME)
        from mbbl.env.gym_env import walker
        self.ENV = walker.env(env_name='gym_ant', rand_seed=1234,
    ...
    def obs_cost_fn(obs):
        ...
        """
        if isinstance(obs, np.ndarray):
            velocity_cost = -obs[:, 13]  # the qvel for the root-x joint
            height_cost = 3 * np.square(obs[:, 0] - 0.57)  # the height
            return velocity_cost + height_cost
        else:
            velocity_cost = -obs[:, 13]  # the qvel for the root-x joint
            height_cost = 3 * tf.square(obs[:, 0] - 0.57)  # the height
            return velocity_cost + height_cost
