...
"""
    Module name, ENV_NAME
    MODEL_IN, MODEL_OUT,
    import env, env_name
"""


class GymINVPendulumConfigModule:
    ENV_NAME = "MBRLGYM_invpendulum-v0"
    TASK_HORIZON = 1000
    NTRAIN_ITERS = 300
    NROLLOUTS_PER_ITER = 1
    PLAN_HOR = 30
    INIT_VAR = 0.25
    MODEL_IN, MODEL_OUT = 5, 4  # obs -> 4, action -> 1
    GP_NINDUCING_POINTS = 300

    def __init__(self):
        # self.ENV = gym.make(self.ENV_NAME)
        from mbbl.env.gym_env import invertedPendulum
        self.ENV = invertedPendulum.env(
            env_name='gym_invertedPendulum', rand_seed=1234,
...
CONFIG_MODULE = GymINVPendulumConfigModule
